...
TXT={"en":{"lang":"Choose language / Sprache wählen","prep":"Preparing temporary environment, reports, downloads and device scan...","ready":"Ready. Select a device and macOS, then press FLASH.","ready_save":"Ready. Select macOS, then press SAVE.","device":"Device","macos":"macOS","advanced":"Advanced options","save_folder":"Save to folder instead of flashing USB","verify":"Verify after flash/save","confirm":"I understand the selected USB drive will be erased","confirm_need":"Please tick the erase confirmation checkbox first.","flash":"FLASH","save":"SAVE","cleanup":"Cleanup","workspace":"Workspace","reports":"Reports","efi":"EFI source","recovery":"Recovery source","browse":"Browse","auto_find":"Auto find","secure":"SecureBootModel","scan":"ScanPolicy","hide":"HideAuxiliary","verbose":"Verbose boot","no_macos":"No macOS version selected.","no_usb":"No USB device selected.","no_efi":"No usable EFI folder was found or selected.","done":"Done.","failed":"Failed.","saving":"Saving EFI and Recovery folder...","flashing":"Flashing...","clean":"Clean error description","download_warn":"Some downloads failed. The tool remains usable; missing components may need manual selection.","admin":"Administrator rights are required. Relaunching elevated...","admin_fail":"Administrator elevation failed or was cancelled.","ok":"Verification passed.","warn":"Verification passed with warnings.","fail":"Verification failed.","cleanup1":"Remove temporary downloads, extracted tools and output? Reports/logs will be kept.","cleanup2":"Remove the full temporary workspace including reports and logs?","cleanup_done":"Temporary files removed. Reports and logs kept.","cleanup_all":"Workspace removed. Close the application now."},"de":{"lang":"Sprache wählen / Choose language","prep":"Temporäre Umgebung, Reports, Downloads und Geräte-Scan werden vorbereitet...","ready":"Bereit. Laufwerk und macOS wählen, dann FLASH drücken.","ready_save":"Bereit. macOS wählen, dann SPEICHERN drücken.","device":"Laufwerk","macos":"macOS","advanced":"Erweiterte Optionen","save_folder":"In Ordner speichern statt USB flashen","verify":"Nach Flash/Speichern überprüfen","confirm":"Ich verstehe, dass das ausgewählte USB-Laufwerk gelöscht wird","confirm_need":"Bitte zuerst das Lösch-Bestätigungskästchen aktivieren.","flash":"FLASH","save":"SPEICHERN","cleanup":"Cleanup","workspace":"Workspace","reports":"Reports","efi":"EFI-Quelle","recovery":"Recovery-Quelle","browse":"Durchsuchen","auto_find":"Automatisch suchen","secure":"SecureBootModel","scan":"ScanPolicy","hide":"HideAuxiliary","verbose":"Verbose Boot","no_macos":"Keine macOS-Version ausgewählt.","no_usb":"Kein USB-Laufwerk ausgewählt.","no_efi":"Kein nutzbarer EFI-Ordner gefunden oder ausgewählt.","done":"Fertig.","failed":"Fehlgeschlagen.","saving":"EFI und Recovery-Ordner werden gespeichert...","flashing":"Flash läuft...","clean":"Saubere Fehlerbeschreibung","download_warn":"Einige Downloads sind fehlgeschlagen. Das Tool bleibt nutzbar; fehlende Bestandteile müssen ggf. manuell gewählt werden.","admin":"Administratorrechte sind nötig. Starte erhöht neu...","admin_fail":"Administrator-Erhöhung fehlgeschlagen oder abgebrochen.","ok":"Überprüfung bestanden.","warn":"Überprüfung mit Warnungen bestanden.","fail":"Überprüfung fehlgeschlagen.","cleanup1":"Temporäre Downloads, extrahierte Tools und Output entfernen? Reports/Logs bleiben erhalten.","cleanup2":"Kompletten temporären Workspace inklusive Reports und Logs entfernen?","cleanup_done":"Temporäre Dateien entfernt. Reports und Logs bleiben erhalten.","cleanup_all":"Workspace entfernt. Anwendung jetzt schließen."}}

def win(): return os.name=="nt"
...
def pythonw():
    if not win(): return sys.executable
...
def elevate():
    ...
    args=[script]+[a for a in sys.argv[1:] if a!="--elevated-child"]+["--elevated-child"]
    if win():
        params=" ".join([f'"{a}"' if " " in a else a for a in args])
        return ctypes.windll.shell32.ShellExecuteW(None,"runas",pythonw(),params,None,1)>32
    if linux():
        pk=shutil.which("pkexec")
        if pk:
            env=[f"DISPLAY={os.environ.get('DISPLAY','')}",f"XAUTHORITY={os.environ.get('XAUTHORITY','')}"]
            subprocess.Popen([pk,"env"]+env+[sys.executable]+args); return True
    return False

if not admin() and "--elevated-child" not in sys.argv and "--no-admin" not in sys.argv:
    if elevate(): sys.exit(0)

BASE=Path(tempfile.gettempdir())/"OpenCoreForge_Runtime"; DOWNLOADS=BASE/"downloads"; EXTRACTED=BASE/"extracted"; TOOLS=BASE/"tools"; OUTPUT=BASE/"output"; REPORTS=BASE/"reports"; LOGS=BASE/"logs"
for p in [DOWNLOADS,EXTRACTED,TOOLS,OUTPUT,REPORTS,LOGS]: p.mkdir(parents=True,exist_ok=True)
        ...
        try:
            ...
            for d in json.loads(r.stdout).get("blockdevices",[]):
                if d.get("type")=="disk" and (str(d.get("tran","")).lower()=="usb" or str(d.get("rm","0"))=="1") and str(d.get("ro","0")) not in ["1","True","true"]:
                    out.append({"platform":"linux","Name":d.get("name"),"Model":d.get("model") or d.get("name"),"Size":d.get("size"),"Path":"/dev/"+d.get("name","")})
        except Exception: pass
        return out
    return []

def report_env():
    data={"app":APP,"version":VER,"python":sys.version,"executable":sys.executable,"admin":admin(),"platform":sys.platform,"workspace":str(BASE)}
    if win(): data["disks"]=psjson("Get-Disk | Select Number,FriendlyName,Model,Size,BusType,PartitionStyle,IsBoot,IsSystem")
    elif linux():
        try: data["lsblk"]=json.loads(subprocess.run(["lsblk","-J","-o","NAME,MODEL,SIZE,TYPE,TRAN,RM,RO,MOUNTPOINT"],capture_output=True,text=True,timeout=30).stdout)
        except Exception: pass
    (REPORTS/"environment_report.json").write_text(json.dumps(data,indent=2,ensure_ascii=False),encoding="utf-8")

def dl(url,target,tries=3):
    headers={"User-Agent":f"Mozilla/5.0 {APP}/{VER}"}; last=""
    for i in range(tries):
        ...
        except Exception as e: last=str(e); time.sleep(1+i)
...
def validate(root):
    ...
    if cfg.exists():
        try:
            pl=plistlib.load(cfg.open("rb")); add("OK","config.plist parsed")
...
class Lang(tk.Toplevel):
    def __init__(self,master,cb):
        super().__init__(master); self.cb=cb; self.title(APP); self.geometry("360x210"); self.resizable(False,False); self.configure(bg="#f3f4f6")
        tk.Label(self,text="Choose language / Sprache wählen",bg="#f3f4f6",font=("Segoe UI",13,"bold")).pack(pady=(24,16))
        r=tk.Frame(self,bg="#f3f4f6"); r.pack(pady=12)
        tk.Button(r,text="🇬🇧  English",width=14,height=2,command=lambda:self.pick("en"),font=("Segoe UI",11)).pack(side="left",padx=10)
        tk.Button(r,text="🇩🇪  Deutsch",width=14,height=2,command=lambda:self.pick("de"),font=("Segoe UI",11)).pack(side="left",padx=10)
    def pick(self,l): self.cb(l); self.destroy()
class Splash(tk.Toplevel):
    def __init__(self,master):
        super().__init__(master); self.title(APP); self.geometry("510x175"); self.resizable(False,False); self.configure(bg="#f3f4f6")
        self.label=tk.Label(self,text="Preparing...",bg="#f3f4f6",font=("Segoe UI",13,"bold")); self.label.pack(pady=(28,16))
        self.pb=ttk.Progressbar(self,mode="indeterminate",length=400); self.pb.pack(pady=8); self.pb.start(8)
        tk.Label(self,text=str(BASE),bg="#f3f4f6",fg="#4b5563",wraplength=460,font=("Segoe UI",9)).pack(pady=8)
...
class App(tk.Tk):
    ...
    def boot(self):
        try:
            ...
            self.put(self.splash.label.config,{"text":"Scanning devices..."}); self.usb=usb_list(); self.find()
        except Exception:
    ...
    def find(self):
        ...
        self.efi=sorted(efis,key=lambda p:len(str(p)))[0] if efis else None; self.recovery=sorted(recs,key=lambda p:len(str(p)))[0] if recs else None
    def ready(self):
        self.splash.destroy(); Lang(self,self.set_lang)
    ...
    def style(self):
        ...
        self.configure(bg="#f3f4f6"); s.configure("TFrame",background="#f3f4f6"); s.configure("TLabel",background="#f3f4f6",font=("Segoe UI",10)); s.configure("Title.TLabel",background="#f3f4f6",font=("Segoe UI",15,"bold")); s.configure("Small.TLabel",background="#f3f4f6",foreground="#4b5563",font=("Segoe UI",9)); s.configure("TButton",padding=(10,6),font=("Segoe UI",10)); s.configure("Flash.TButton",padding=(18,8),font=("Segoe UI",10,"bold"))
    def build(self):
        self.style(); self.title(f"{APP} {VER}"); self.geometry("560x500"); self.minsize(520,430); self.resizable(True,True); self.protocol("WM_DELETE_WINDOW",self.close)
        m=ttk.Frame(self,padding=16); m.pack(fill="both",expand=True)
        ttk.Label(m,text=APP,style="Title.TLabel").grid(row=0,column=0,sticky="w"); self.sub=ttk.Label(m,text=self.t("ready"),style="Small.TLabel"); self.sub.grid(row=1,column=0,sticky="w",pady=(0,10))
        form=ttk.Frame(m); form.grid(row=2,column=0,sticky="ew"); form.columnconfigure(1,weight=1)
        ttk.Label(form,text=self.t("device")).grid(row=0,column=0,sticky="w",padx=(0,12),pady=6); self.devvar=tk.StringVar(); self.devbox=ttk.Combobox(form,textvariable=self.devvar,state="readonly",width=50); self.devbox.grid(row=0,column=1,sticky="ew",pady=6); self.devbox.bind("<<ComboboxSelected>>",lambda e:self.seldev())
        ttk.Label(form,text=self.t("macos")).grid(row=1,column=0,sticky="w",padx=(0,12),pady=6); self.osvar=tk.StringVar(); self.osbox=ttk.Combobox(form,textvariable=self.osvar,state="readonly",width=50); self.osbox.grid(row=1,column=1,sticky="ew",pady=6); self.osbox.bind("<<ComboboxSelected>>",lambda e:self.selos())
        self.advopen=tk.BooleanVar(False); self.advbtn=ttk.Checkbutton(m,text="▼ "+self.t("advanced"),variable=self.advopen,command=self.toggle); self.advbtn.grid(row=3,column=0,sticky="w",pady=(8,2)); self.adv=ttk.Frame(m)
        ...
        ttk.Checkbutton(self.adv,text=self.t("save_folder"),variable=self.savefolder,command=self.updatebtn).grid(row=0,column=0,columnspan=3,sticky="w",pady=3)
        self.opt(1,self.t("secure"),self.secure,["Disabled","Default"]); self.opt(2,self.t("scan"),self.scan,["0","Default"]); self.opt(3,self.t("hide"),self.hide,["False","True"]); ttk.Checkbutton(self.adv,text=self.t("verbose"),variable=self.verbose).grid(row=4,column=1,sticky="w",pady=3)
        ttk.Label(self.adv,text=self.t("efi")).grid(row=5,column=0,sticky="w",pady=3); self.efil=ttk.Label(self.adv,text=str(self.efi) if self.efi else "-",width=38); self.efil.grid(row=5,column=1,sticky="ew",pady=3); ttk.Button(self.adv,text=self.t("browse"),command=self.pickefi).grid(row=5,column=2,padx=4)
        ttk.Label(self.adv,text=self.t("recovery")).grid(row=6,column=0,sticky="w",pady=3); self.recl=ttk.Label(self.adv,text=str(self.recovery) if self.recovery else "-",width=38); self.recl.grid(row=6,column=1,sticky="ew",pady=3); ttk.Button(self.adv,text=self.t("browse"),command=self.pickrec).grid(row=6,column=2,padx=4); ttk.Button(self.adv,text=self.t("auto_find"),command=self.refreshfind).grid(row=7,column=1,sticky="w",pady=4)
        self.status=tk.Text(m,height=7,width=64,bg="#111827",fg="#e5e7eb",relief="flat",font=("Consolas",9),wrap="word"); self.status.grid(row=5,column=0,sticky="ew",pady=(10,8))
        b=ttk.Frame(m); b.grid(row=6,column=0,sticky="ew"); b.columnconfigure(1,weight=1); self.verify=tk.BooleanVar(True); self.confirm=tk.BooleanVar(False); checks=ttk.Frame(b); checks.grid(row=0,column=0,sticky="w"); ttk.Checkbutton(checks,text=self.t("verify"),variable=self.verify).pack(anchor="w"); ttk.Checkbutton(checks,text=self.t("confirm_erase"),variable=self.confirm).pack(anchor="w")
        ttk.Button(b,text=self.t("cleanup"),command=self.cleanup).grid(row=0,column=1,sticky="e",padx=4); ttk.Button(b,text=self.t("reports"),command=lambda:self.open(REPORTS)).grid(row=0,column=2,sticky="e",padx=4); ttk.Button(b,text=self.t("workspace"),command=lambda:self.open(BASE)).grid(row=0,column=3,sticky="e",padx=4); self.flashbtn=ttk.Button(b,text=self.t("flash"),style="Flash.TButton",command=self.flash); self.flashbtn.grid(row=0,column=4,sticky="e")
        self.populate(); self.log(self.t("ready"))
    def opt(self,r,l,var,vals): ttk.Label(self.adv,text=l).grid(row=r,column=0,sticky="w",pady=3); ttk.Combobox(self.adv,textvariable=var,values=vals,state="readonly",width=18).grid(row=r,column=1,sticky="w",pady=3)
    def toggle(self): self.adv.grid(row=4,column=0,sticky="ew",pady=4) if self.advopen.get() else self.adv.grid_remove(); self.advbtn.config(text=("▲ " if self.advopen.get() else "▼ ")+self.t("advanced"))
    ...
    def populate(self):
        vals=[]
        for d in self.usb: vals.append(f"Disk {d.get('Number')} | {d.get('FriendlyName')} | {human(d.get('Size',0))} | {d.get('BusType')}" if d.get("platform")=="windows" else f"{d.get('Path')} | {d.get('Model')} | {human(d.get('Size',0))}")
        self.devbox["values"]=vals
        if vals: self.devvar.set(vals[0]); self.seldev()
        self.osbox["values"]=[f"{a} ({b}) - {c}" for a,b,c in MACOS]
    def seldev(self):
        i=self.devbox.current(); self.selected_usb=self.usb[i] if 0<=i<len(self.usb) else None
    def selos(self):
        i=self.osbox.current(); self.selected_macos=MACOS[i] if 0<=i<len(MACOS) else None
    ...
    def flash(self):
        ...
        if self.savefolder.get():
            ...
            return
        if not self.selected_usb: messagebox.showwarning(APP,self.t("no_device")); return
        if not admin():
            messagebox.showinfo(APP,self.t("admin_needed"))
            if elevate(): self.close()
            else: messagebox.showerror(APP,self.t("admin_failed"))
            return
        if not self.confirm.get(): messagebox.showwarning(APP,self.t("confirm_needed")); return
        threading.Thread(target=self.flash_worker,daemon=True).start()
    def save_to(self,target):
        try:
            self.put(self.log,self.t("saving")); self.copy_payload(target); self.patch(target); self.do_report(target) if self.verify.get() else None; self.put(self.log,self.t("done"))
        except Exception: self.err()
    def flash_worker(self):
        try:
            self.put(self.log,self.t("flashing")); target=self.format_usb(); self.copy_payload(target); self.patch(target); self.do_report(target) if self.verify.get() else None; self.put(self.log,self.t("done"))
        except Exception: self.err()
    def copy_payload(self,target):
        ...
        if self.recovery and Path(self.recovery).exists():
            rec=target/"com.apple.recovery.boot"; shutil.rmtree(rec,ignore_errors=True); shutil.copytree(self.recovery,rec)
    def do_report(self,target):
        status,rows=validate(target); rp=REPORTS/("validation_"+time.strftime("%Y%m%d_%H%M%S")+".txt"); lines=[f"{APP} {VER}",f"Target: {target}",f"macOS: {self.selected_macos}",""]+[f"[{a}] {b}" for a,b in rows]; rp.write_text("\n".join(lines),encoding="utf-8"); self.put(self.log,"\n".join(lines[-len(rows):])); msg=self.t("verify_ok") if status=="passed" else self.t("verify_warn") if status=="warnings" else self.t("verify_fail"); self.put(messagebox.showinfo if status!="failed" else messagebox.showerror,APP,msg+"\n\n"+str(rp))
    def format_usb(self):
        if win():
            ...
            letter=[x.strip().replace(":","") for x in r.stdout.splitlines() if x.strip()][-1]; target=Path(f"{letter}:/")
            for _ in range(30):
                if target.exists(): return target
                time.sleep(.25)
            raise RuntimeError(f"Drive {letter}: not available")
        if linux():
    ...
    def err(self):
        err=traceback.format_exc(); (LOGS/"operation_error.log").write_text(err,encoding="utf-8"); clean=err.splitlines()[-1]; self.put(self.log,self.t("failed")+" "+self.t("clean_error")+": "+clean); self.put(messagebox.showerror,APP,self.t("failed")+"\n\n"+clean)
    def open(self,p):
        if win(): os.startfile(str(p))
        elif linux(): subprocess.Popen(["xdg-open",str(p)])
        else: subprocess.Popen(["open",str(p)])
    def cleanup(self):
        if messagebox.askyesno(APP,self.t("cleanup_temp_confirm")):
        ...
        if messagebox.askyesno(APP,self.t("cleanup_all_confirm")):
            shutil.rmtree(BASE,ignore_errors=True); messagebox.showinfo(APP,self.t("cleanup_all_done"))
