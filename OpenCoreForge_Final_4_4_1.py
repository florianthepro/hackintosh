...
def elevate_before_anything():
    ...
    if is_win():
        ...
        return ctypes.windll.shell32.ShellExecuteW(None,"runas",python_gui_exe(),params,None,1)>32
    if is_linux():
...
# absolutely first: elevate before workspace/downloads/scan/gui
if not is_admin() and "--elevated" not in sys.argv and "--no-admin" not in sys.argv:
    if elevate_before_anything(): sys.exit(0)

BASE=Path(tempfile.gettempdir())/"OpenCoreForge_Runtime"
DOWNLOADS=BASE/"downloads"; EXTRACTED=BASE/"extracted"; TOOLS=BASE/"tools"; OUTPUT=BASE/"output"; REPORTS=BASE/"reports"; LOGS=BASE/"logs"
...
def psjson(cmd):
    ...
    try:
        r=subprocess.run(["powershell","-NoProfile","-ExecutionPolicy","Bypass","-Command",cmd+" | ConvertTo-Json -Depth 6"],capture_output=True,text=True,timeout=90)
...
def write_env_report():
    data={"app":APP,"version":VER,"admin":is_admin(),"python":sys.version,"executable":sys.executable,"platform":sys.platform,"workspace":str(BASE)}
...
def validate(root):
    ...
    drivers=root/"EFI"/"OC"/"Drivers"; add("OK" if drivers.exists() and any(x.name.lower() in ["hfsplus.efi","openhfsplus.efi"] for x in drivers.iterdir()) else "WARN","HFS driver")
    rec=root/"com.apple.recovery.boot"; chk(rec,"Recovery folder",False)
    if rec.exists(): chk(rec/"BaseSystem.dmg","BaseSystem.dmg"); chk(rec/"BaseSystem.chunklist","BaseSystem.chunklist",False)
    cfg=root/"EFI"/"OC"/"config.plist"
    if cfg.exists():
        try:
            with cfg.open("rb") as f: pl=plistlib.load(f)
            add("OK","config.plist parsed"); add("OK" if pl.get("Misc",{}).get("Security",{}).get("ScanPolicy")==0 else "WARN","ScanPolicy check"); add("OK" if pl.get("Misc",{}).get("Boot",{}).get("HideAuxiliary") is False else "WARN","HideAuxiliary check")
...
class App(tk.Tk):
    def __init__(self):
        super().__init__(); self.lang="en"; self.usb=[]; self.efi=None; self.recovery=None; self.selected_usb=None; self.selected_macos=None; self.errors=[]; self.q=queue.Queue(); self.load_state(); self.setup(); self.show_language(); self.after(80,self.drain)
    ...
    def t(self,k):
        aliases={"confirm_erase":"confirm","verify_ok":"ok","verify_warn":"warn","verify_fail":"fail","cleanup_temp_confirm":"cleanup1","cleanup_all_confirm":"cleanup2"}
        k=aliases.get(k,k)
        return TXT.get(self.lang,TXT["en"]).get(k,k)
    def load_state(self):
        try:
    ...
    def setup(self):
        ...
        self.title(f"{APP} {VER}"); self.geometry("500x360"); self.minsize(480,340); self.resizable(True,True); self.protocol("WM_DELETE_WINDOW",self.close)
    def clear(self):
        for w in self.winfo_children(): w.destroy()
    def show_language(self):
        self.clear(); m=ttk.Frame(self,padding=18); m.pack(fill="both",expand=True); ttk.Label(m,text=APP,style="Title.TLabel").pack(anchor="w"); ttk.Label(m,text=TXT["en"]["lang"],style="Small.TLabel").pack(anchor="w",pady=(0,22)); r=ttk.Frame(m); r.pack(expand=True); ttk.Button(r,text="English",command=lambda:self.start("en")).pack(side="left",padx=10,ipadx=8,ipady=8); ttk.Button(r,text="Deutsch",command=lambda:self.start("de")).pack(side="left",padx=10,ipadx=8,ipady=8)
    def start(self,lang):
        self.lang=lang; self.save_state(); self.show_prepare(); threading.Thread(target=self.bootstrap,daemon=True).start()
    def show_prepare(self):
        self.clear(); m=ttk.Frame(self,padding=18); m.pack(fill="both",expand=True); ttk.Label(m,text=APP,style="Title.TLabel").pack(anchor="w"); self.prep_label=ttk.Label(m,text=self.t("prep")); self.prep_label.pack(anchor="w",pady=20); pb=ttk.Progressbar(m,mode="indeterminate"); pb.pack(fill="x",pady=8); pb.start(8); self.prep_status=ttk.Label(m,text=str(BASE),style="Small.TLabel",wraplength=440); self.prep_status.pack(anchor="w",pady=8)
    ...
    def bootstrap(self):
        try:
            write_env_report(); self.downloads(); self.usb=usb_list(); self.find()
        except Exception:
    ...
    def build_main(self):
        self.clear(); self.geometry("500x390"); m=ttk.Frame(self,padding=10); m.pack(fill="both",expand=True); ttk.Label(m,text=APP,style="Title.TLabel").grid(row=0,column=0,sticky="w"); self.sub=ttk.Label(m,text=self.t("ready"),style="Small.TLabel"); self.sub.grid(row=1,column=0,sticky="w",pady=(0,6)); form=ttk.Frame(m); form.grid(row=2,column=0,sticky="ew"); form.columnconfigure(1,weight=1)
        ttk.Label(form,text=self.t("device")).grid(row=0,column=0,sticky="w",padx=(0,8),pady=4); self.devvar=tk.StringVar(); self.devbox=ttk.Combobox(form,textvariable=self.devvar,state="readonly",width=38); self.devbox.grid(row=0,column=1,sticky="ew",pady=4); self.devbox.bind("<<ComboboxSelected>>",lambda e:self.seldev())
        ttk.Label(form,text=self.t("macos")).grid(row=1,column=0,sticky="w",padx=(0,8),pady=4); self.osvar=tk.StringVar(); self.osbox=ttk.Combobox(form,textvariable=self.osvar,state="readonly",width=38); self.osbox.grid(row=1,column=1,sticky="ew",pady=4); self.osbox.bind("<<ComboboxSelected>>",lambda e:self.selos())
        self.advopen=tk.BooleanVar(False); self.advbtn=ttk.Checkbutton(m,text="▼ "+self.t("advanced"),variable=self.advopen,command=self.toggle); self.advbtn.grid(row=3,column=0,sticky="w",pady=(5,0)); self.adv=ttk.Frame(m); self.savefolder=tk.BooleanVar(False); self.secure=tk.StringVar(value="Disabled"); self.scan=tk.StringVar(value="0"); self.hide=tk.StringVar(value="False"); self.verbose=tk.BooleanVar(value=True)
        ttk.Checkbutton(self.adv,text=self.t("save_folder"),variable=self.savefolder,command=self.updatebtn).grid(row=0,column=0,columnspan=3,sticky="w"); self.opt(1,self.t("secure"),self.secure,["Disabled","Default"]); self.opt(2,self.t("scan"),self.scan,["0","Default"]); self.opt(3,self.t("hide"),self.hide,["False","True"]); ttk.Checkbutton(self.adv,text=self.t("verbose"),variable=self.verbose).grid(row=4,column=1,sticky="w")
        ...
        ttk.Label(self.adv,text=self.t("recovery")).grid(row=6,column=0,sticky="w"); self.recl=ttk.Label(self.adv,text=str(self.recovery) if self.recovery else "-",width=28); self.recl.grid(row=6,column=1,sticky="ew"); ttk.Button(self.adv,text=self.t("browse"),command=self.pickrec).grid(row=6,column=2,padx=3); ttk.Button(self.adv,text=self.t("auto_find"),command=self.refreshfind).grid(row=7,column=1,sticky="w")
        self.status=tk.Text(m,height=5,width=55,bg="#111827",fg="#e5e7eb",relief="flat",font=("Consolas",8),wrap="word"); self.status.grid(row=5,column=0,sticky="nsew",pady=(7,6)); m.rowconfigure(5,weight=1); b=ttk.Frame(m); b.grid(row=6,column=0,sticky="ew"); b.columnconfigure(1,weight=1); self.verify=tk.BooleanVar(True); self.confirm=tk.BooleanVar(False); ch=ttk.Frame(b); ch.grid(row=0,column=0,sticky="w"); ttk.Checkbutton(ch,text=self.t("verify"),variable=self.verify).pack(anchor="w"); ttk.Checkbutton(ch,text=self.t("confirm"),variable=self.confirm).pack(anchor="w")
        ttk.Button(b,text=self.t("cleanup"),command=self.cleanup).grid(row=0,column=1,sticky="e",padx=2); ttk.Button(b,text=self.t("reports"),command=lambda:self.open(REPORTS)).grid(row=0,column=2,sticky="e",padx=2); self.flashbtn=ttk.Button(b,text=self.t("flash"),style="Flash.TButton",command=self.flash); self.flashbtn.grid(row=0,column=3,sticky="e"); self.populate(); self.log(self.t("ready")); [self.log(x) for x in ([self.t("download_warn")]+self.errors if self.errors else [])]
