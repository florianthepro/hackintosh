...
APP_TITLE = "OpenCore OneClick Builder"
...
MACOS_ITEMS = [
    ("High Sierra", "10.13", "Legacy", "Für ältere Systeme/NVIDIA-Legacy relevant"),
    ("Mojave", "10.14", "Legacy", "Ältere stabile Basis"),
    ("Catalina", "10.15", "Stabil", "Gute ältere Intel-Basis"),
    ("Big Sur", "11", "Stabil", "USB-Mapping wichtig"),
    ("Monterey", "12", "Empfohlen", "Oft stabil für Intel-Systeme"),
    ("Ventura", "13", "Modern", "Gut mit vielen AMD RX GPUs"),
    ("Sonoma", "14", "Modern", "Mehr Feinschliff/Patches möglich"),
    ("Sequoia", "15", "Aktuell", "Kompatibilität genau prüfen"),
    ("Tahoe", "26", "Experimentell", "Nur wenn Tools/Hardware passen")
]
...
try:
    ...
    fatal("Tkinter fehlt. Installiere Python von python.org neu und aktiviere 'tcl/tk and IDLE'. Fehler: " + repr(e))
    raise SystemExit(1)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.banner_msg("Bereit. Klicke links/oben auf 'Alles vorbereiten'.", "ok")
        if is_windows() and not is_admin(): self.banner_msg("Warnung: Nicht als Administrator gestartet. USB-Formatieren braucht Adminrechte.", "warn")

    def report_callback_exception(self, exc, val, tb):
        text = "".join(traceback.format_exception(exc, val, tb)); elog(text); self.log("FEHLER: " + str(val))
        try: messagebox.showerror("Fehler", "Fehler abgefangen, Fenster bleibt offen.\n\n" + text[-3000:])
        except Exception: pass
    ...
    def build_ui(self):
        ...
        self.frames = {}
        for key,label in [("home","Start"),("hardware","Hardware"),("macos","macOS"),("tools","Tools"),("usb","USB"),("write","Schreiben"),("help","Boot-Hilfe"),("logs","Logs")]:
            ttk.Button(side, text=label, style="Nav.TButton", command=lambda k=key:self.show(k)).pack(fill="x", pady=4)
        ttk.Button(side, text="Workspace öffnen", style="Nav.TButton", command=self.open_workspace).pack(fill="x", pady=(24,4))
    ...
    def show(self,key):
        titles={"home":("Start","Geführter Ablauf."),"hardware":("Hardware","Hardware erkennen."),"macos":("macOS","Version wählen."),"tools":("Tools","GitHub-Tools und Terminal."),"usb":("USB","USB sicher vorbereiten."),"write":("Schreiben","EFI/Recovery kopieren."),"help":("Boot-Hilfe","Troubleshooting."),"logs":("Logs","Fehler und Meldungen.")}
    ...
    def make_home(self):
        ...
        ttk.Label(c,text="Geführter Ablauf",font=("Segoe UI",14,"bold"),background="#ffffff").pack(anchor="w")
        ttk.Label(c,text="Erst vorbereiten, dann Hardware scannen, macOS wählen, USB vorbereiten und schreiben.",background="#ffffff",foreground="#334155").pack(anchor="w",pady=8)
        row=ttk.Frame(c,style="Card.TFrame"); row.pack(fill="x")
        ttk.Button(row,text="1. Alles vorbereiten",style="Accent.TButton",command=self.autoprepare).pack(side="left",padx=4)
        ttk.Button(row,text="2. Hardware scannen",command=lambda:[self.show("hardware"),self.scan_hardware()]).pack(side="left",padx=4)
        ttk.Button(row,text="3. macOS wählen",command=lambda:self.show("macos")).pack(side="left",padx=4)
        ttk.Button(row,text="4. USB wählen",command=lambda:[self.show("usb"),self.refresh_disks()]).pack(side="left",padx=4)
        ttk.Button(row,text="5. Schreiben",command=lambda:self.show("write")).pack(side="left",padx=4)
    ...
    def make_hardware(self):
        f=self.page("hardware"); top=self.card(f); top.pack(fill="x"); ttk.Button(top,text="Hardware scannen",style="Accent.TButton",command=self.scan_hardware).pack(side="left"); ttk.Button(top,text="JSON speichern",command=self.save_hardware).pack(side="left",padx=8)
    ...
    def make_tools(self):
        ...
        ttk.Button(top,text="Tools laden",style="Accent.TButton",command=self.download_extract_all).pack(side="left",padx=4)
        ...
        inp=self.card(f,8); inp.pack(fill="x"); self.stdin_var=tk.StringVar(); ent=ttk.Entry(inp,textvariable=self.stdin_var); ent.pack(side="left",fill="x",expand=True,padx=(0,8)); ent.bind("<Return>",lambda e:self.send_stdin()); ttk.Button(inp,text="Senden",command=self.send_stdin).pack(side="left")
    def make_usb(self):
        f=self.page("usb"); top=self.card(f); top.pack(fill="x")
        ttk.Button(top,text="Aktualisieren",style="Accent.TButton",command=self.refresh_disks).pack(side="left",padx=4)
        ttk.Button(top,text="USB löschen + GPT/FAT32",style="Danger.TButton",command=self.prepare_usb).pack(side="left",padx=4)
        ttk.Label(top,text="Buchstabe:",background="#fff").pack(side="left",padx=(18,4)); self.letter_var=tk.StringVar(value=self.usb_letter); ttk.Entry(top,width=5,textvariable=self.letter_var).pack(side="left")
        self.disk_tree=ttk.Treeview(f,columns=("num","name","size","bus","part","model"),show="headings")
        for col,title,width in [("num","Disk",70),("name","Name",220),("size","Größe",110),("bus","Bus",80),("part","Partition",100),("model","Modell",520)]: self.disk_tree.heading(col,text=title); self.disk_tree.column(col,width=width)
        self.disk_tree.pack(fill="both",expand=True,pady=12); self.disk_tree.bind("<<TreeviewSelect>>",self.on_disk_select); self.usb_info=ttk.Label(f,text="Kein USB ausgewählt."); self.usb_info.pack(anchor="w")
    def make_write(self):
        f=self.page("write"); c=self.card(f); c.pack(fill="x")
        ttk.Button(c,text="EFI wählen",command=self.pick_efi).grid(row=0,column=0,padx=4,pady=4,sticky="w"); self.efi_lbl=ttk.Label(c,text="keine EFI",background="#fff"); self.efi_lbl.grid(row=0,column=1,sticky="w")
        ttk.Button(c,text="Recovery wählen",command=self.pick_recovery).grid(row=1,column=0,padx=4,pady=4,sticky="w"); self.rec_lbl=ttk.Label(c,text="kein Recovery",background="#fff"); self.rec_lbl.grid(row=1,column=1,sticky="w")
        row=ttk.Frame(c,style="Card.TFrame"); row.grid(row=2,column=0,columnspan=2,sticky="w",pady=8)
        ttk.Button(row,text="Automatisch suchen",command=self.auto_find).pack(side="left",padx=4); ttk.Button(row,text="Auf USB kopieren",style="Accent.TButton",command=self.copy_to_usb).pack(side="left",padx=4); ttk.Button(row,text="Sanity-Check",command=self.sanity_dialog).pack(side="left",padx=4); ttk.Button(row,text="Verbose Boot setzen",command=self.set_verbose).pack(side="left",padx=4)
    ...
    def make_help(self):
        f=self.page("help"); c=self.card(f); c.pack(fill="x"); txt="""Wenn Apple-Logo/Ladebalken hängt:\n1. Reset NVRAM im OpenCore-Menü.\n2. Verbose: -v keepsyms=1 debug=0x100.\n3. ScanPolicy=0, HideAuxiliary=False, SecureBootModel=Disabled.\n4. OpenRuntime.efi + HfsPlus.efi/OpenHfsPlus.efi prüfen.\n5. Nach Änderungen OC Snapshot/ocvalidate.\n6. BIOS: Secure Boot/Fast Boot/CSM aus, AHCI an."""; ttk.Label(c,text=txt,background="#fff",justify="left",wraplength=1000).pack(anchor="w")
    def make_logs(self):
        f=self.page("logs"); top=self.card(f); top.pack(fill="x"); ttk.Button(top,text="Log speichern",command=self.save_log).pack(side="left",padx=4); ttk.Button(top,text="Crash-Log öffnen",command=self.open_crash).pack(side="left",padx=4); ttk.Button(top,text="Leeren",command=lambda:self.log_box.delete("1.0","end")).pack(side="left",padx=4)
    ...
    def task(self,name,fn):
        def run():
            self.q(self.progress.start,8); self.q(self.banner_msg,name+" läuft...","info")
            try: fn(); self.q(self.banner_msg,name+" fertig.","ok")
            except Exception:
                text=traceback.format_exc(); elog(text); self.q(self.log,text); self.q(self.banner_msg,name+" fehlgeschlagen. Siehe Logs.","err")
    ...
    def ensure_env(self):
        self.q(self.log,"Prüfe pip/venv..."); subprocess.run([sys.executable,"-m","ensurepip","--upgrade"],capture_output=True,text=True,timeout=180); subprocess.run([sys.executable,"-m","pip","install","--upgrade","pip","setuptools","wheel"],capture_output=True,text=True,timeout=300)
        if not VENV.exists(): subprocess.run([sys.executable,"-m","venv",str(VENV)],capture_output=True,text=True,timeout=300)
    def download_extract_all(self): self.task("Tools laden",self.download_extract)
    def download_extract(self):
        for name,url in SOURCES.items():
            ext=".py" if url.endswith(".py") else ".zip"; target=DOWNLOADS/(clean(name)+ext)
            if target.exists() and target.stat().st_size>0: self.q(self.log,"Schon vorhanden: "+target.name); continue
            self.q(self.log,"Lade "+name+"...")
        ...
        for z in DOWNLOADS.glob("*.zip"):
            ...
            try:
                ...
                self.q(self.log,"Entpackt: "+z.name)
            except Exception as e: self.q(self.log,"Entpacken fehlgeschlagen "+z.name+": "+str(e))
    ...
    def install_requirements(self,sync=False):
        def work():
            py=self.pyexe(); self.q(self.log,"Nutze Python: "+py); subprocess.run([py,"-m","ensurepip","--upgrade"],capture_output=True,text=True,timeout=180); subprocess.run([py,"-m","pip","install","--upgrade","pip","setuptools","wheel"],capture_output=True,text=True,timeout=300)
            reqs=list(EXTRACTED.rglob("requirements.txt")); self.q(self.log,"requirements gefunden: "+str(len(reqs)))
            for req in reqs:
    ...
    def scan_hardware(self):
        def work(): data=get_hw(); self.hardware=data; self.q(self.hw_box.delete,"1.0","end"); self.q(self.hw_box.insert,"end",json.dumps(data,indent=2,ensure_ascii=False)); self.q(self.render_macos); self.q(self.log,"Hardware-Scan fertig.")
        self.task("Hardware-Scan",work)
    def save_hardware(self): p=LOGS/"hardware.json"; p.write_text(json.dumps(self.hardware,indent=2,ensure_ascii=False),encoding="utf-8"); messagebox.showinfo("Gespeichert",str(p))
    ...
    def choose_macos(self,item): self.selected_macos=item; self.log(f"Ausgewählt: macOS {item[0]} {item[1]}"); self.show("tools")
    def start_proc(self,cmd,cwd=None):
        if self.proc and self.proc.poll() is None: messagebox.showwarning("Läuft","Bitte erst Stop drücken."); return
    ...
    def run_opcore(self):
        ...
        if not scripts: messagebox.showwarning("Fehlt","Bitte zuerst Tools laden."); return
    ...
    def run_gibmacos(self):
        ...
        else: messagebox.showwarning("Fehlt","gibMacOS nicht gefunden.")
    def run_usbtoolbox(self):
        ...
        else: messagebox.showwarning("Fehlt","USBToolBox nicht gefunden.")
    def refresh_disks(self):
        def work():
            ...
            self.q(fill); self.q(self.log,f"{len(disks)} USB/SD/MMC Laufwerk(e) gefunden.")
        self.task("USB-Liste",work)
    def on_disk_select(self,e=None):
        sel=self.disk_tree.selection();
        if sel: vals=self.disk_tree.item(sel[0],"values"); self.selected_disk=str(vals[0]); self.usb_info.config(text=f"Ausgewählt: Disk {vals[0]} | {vals[1]} | {vals[2]} | {vals[3]}")
    def prepare_usb(self):
        if not is_windows(): messagebox.showerror("Nur Windows","USB-Formatierung nur unter Windows."); return
        if not is_admin(): messagebox.showerror("Adminrechte","Bitte als Administrator starten."); return
        if not self.selected_disk: messagebox.showwarning("Kein USB","Wähle zuerst ein USB-Laufwerk."); return
        letter=self.letter_var.get().strip().upper().replace(":","") or "O"
        if len(letter)!=1 or not letter.isalpha(): messagebox.showerror("Buchstabe","Nur einen Buchstaben, z.B. O."); return
        if simpledialog.askstring("Bestätigung",f"Disk {self.selected_disk} wird gelöscht. Tippe LÖSCHEN:")!="LÖSCHEN": self.log("Abgebrochen."); return
        self.usb_letter=letter; self.save_state()
        def work():
            script=f"select disk {self.selected_disk}\ndetail disk\nclean\nconvert gpt\ncreate partition primary\nformat fs=fat32 quick label=OPENCORE\nassign letter={letter}\nexit\n"; sp=BASE/"diskpart_prepare.txt"; sp.write_text(script,encoding="utf-8"); r=subprocess.run(["diskpart","/s",str(sp)],capture_output=True,text=True,errors="replace"); self.q(self.wlog,r.stdout); self.q(self.wlog,f"USB vorbereitet: {letter}:\\")
        self.task("USB vorbereiten",work)
    def pick_efi(self):
        p=filedialog.askdirectory(title="EFI wählen")
    ...
    def auto_find(self):
        def work():
            ...
            def apply():
                if efis: self.efi_path=sorted(efis,key=lambda x:len(str(x)))[0]; self.efi_lbl.config(text=str(self.efi_path)); self.wlog("EFI gefunden: "+str(self.efi_path))
                else: self.wlog("Keine EFI gefunden.")
                if recs: self.recovery_path=sorted(recs,key=lambda x:len(str(x)))[0]; self.rec_lbl.config(text=str(self.recovery_path)); self.wlog("Recovery gefunden: "+str(self.recovery_path))
                else: self.wlog("Kein Recovery gefunden.")
            self.q(apply)
    ...
    def copy_to_usb(self):
        ...
        if not target.exists(): messagebox.showerror("Nicht gefunden",f"{target} existiert nicht."); return
        if not self.efi_path or not self.efi_path.exists(): messagebox.showwarning("EFI fehlt","EFI wählen."); return
        def work():
            dst=target/"EFI"; 
            if dst.exists(): shutil.rmtree(dst)
            shutil.copytree(self.efi_path,dst); self.q(self.wlog,"EFI kopiert.")
            if self.recovery_path and self.recovery_path.exists():
                rdst=target/"com.apple.recovery.boot"; 
                if rdst.exists(): shutil.rmtree(rdst)
                shutil.copytree(self.recovery_path,rdst); self.q(self.wlog,"Recovery kopiert.")
    ...
    def sanity(self,target):
        ...
        for p,n in checks: self.wlog(("OK    " if p.exists() else "FEHLT ")+n)
        self.wlog(("OK    " if hfs else "FEHLT ")+"HfsPlus.efi/OpenHfsPlus.efi")
    def set_verbose(self):
        if not self.efi_path: self.pick_efi()
        if not self.efi_path: return
        cfg=self.efi_path/"OC"/"config.plist"
        if not cfg.exists(): messagebox.showwarning("Fehlt",str(cfg)); return
        shutil.copy2(cfg,cfg.with_suffix(".plist.verbosebak")); s=cfg.read_text(encoding="utf-8",errors="ignore")
        if "-v keepsyms=1 debug=0x100" in s: self.wlog("Verbose schon vorhanden."); return
        import re
        if "<key>boot-args</key>" in s: cfg.write_text(re.sub(r"(<key>boot-args</key>\s*<string>)(.*?)(</string>)", lambda m:m.group(1)+"-v keepsyms=1 debug=0x100 "+m.group(2)+m.group(3), s, count=1, flags=re.S),encoding="utf-8"); self.wlog("Verbose gesetzt.")
        else: self.wlog("boot-args nicht gefunden.")
    def save_log(self):
        p=LOGS/(time.strftime("log_%Y%m%d_%H%M%S")+".txt"); p.write_text(self.log_box.get("1.0","end"),encoding="utf-8"); messagebox.showinfo("Gespeichert",str(p))
    ...
    def close(self):
        ...
        if self.proc and self.proc.poll() is None:
            if not messagebox.askyesno("Schließen","Tool läuft noch. Trotzdem schließen?"): return
...
def ps_json(cmd):
    ...
    except Exception as e: elog("PS Fehler: "+repr(e)); return []
def get_hw():
    if not is_windows(): return {"system":[sys.platform],"note":"Windows-Scan optimiert."}
...
def score_macos(name,cpu,gpu):
    t=(cpu+" "+gpu).lower()
    if not t.strip(): return "Unbekannt","Erst Hardware scannen."
    if ("rtx" in t or "gtx 10" in t or "gtx 16" in t or "nvidia" in t) and name in ["Ventura","Sonoma","Sequoia","Tahoe"]: return "Problematisch","Viele neuere NVIDIA-GPUs sind ungeeignet."
    if "ryzen" in t or "threadripper" in t: return "Möglich","AMD benötigt Kernel-Patches."
    if any(x in t for x in ["rx 560","rx 570","rx 580","rx 590","rx 5500","rx 5600","rx 5700","rx 6600","rx 6800","rx 6900","radeon pro","navi"]): return "Gut","AMD RX/Radeon erkannt."
    if "intel" in t: return "Möglich/Gut","Intel erkannt; iGPU/SMBIOS prüfen."
    return "Unbekannt","Keine klare Regel."

if __name__ == "__main__":
    try: App().mainloop()
    except Exception:
        text=traceback.format_exc(); elog(text); fatal("Unerwarteter Startfehler:\n\n"+text+"\nLog: "+str(CRASH_LOG))
