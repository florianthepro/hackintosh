...
TEXT = {
    "en": {
        "prep": "Preparing temporary workspace, tools, device report and scan...",
        "device": "Device",
        "macos": "macOS",
        ...
        "flash": "FLASH",
        "save": "SAVE",
        "status_ready": "Ready. Select device and macOS, then press FLASH.",
        "status_save_ready": "Ready. Select macOS, then press SAVE.",
        "status_flash": "Flashing...",
        "status_save": "Saving EFI and Recovery folder...",
        "status_done": "Done.",
        "status_fail": "Failed.",
        "efi": "EFI source",
        ...
        "admin": "Administrator rights are required. Restarting as administrator...",
        ...
        "download_warn": "Some downloads failed. The tool stays usable, but missing components may need manual selection.",
        "saved_to": "Saved to folder",
        "cleanup_done": "Temporary files removed. Reports and logs kept.",
        "cleanup_all_done": "Workspace removed. Close the tool now.",
        "cleanup_confirm": "Remove temporary downloads/extracted tools? Reports and logs will be kept.",
        "cleanup_all_confirm": "Remove entire workspace including reports and logs?",
        "open_reports": "Open reports"
    },
    "de": {
        "prep": "Temporärer Workspace, Tools, Gerätebericht und Scan werden vorbereitet...",
        "device": "Laufwerk",
        "macos": "macOS",
        ...
        "status_save_ready": "Bereit. macOS wählen, dann SPEICHERN drücken.",
        "status_flash": "Flash läuft...",
        "status_save": "EFI und Recovery-Ordner werden gespeichert...",
        "status_done": "Fertig.",
        ...
        "hide": "HideAuxiliary",
        "verbose": "Verbose Boot",
        "admin": "Administratorrechte sind nötig. Neustart als Administrator...",
        ...
        "cleanup_all_done": "Workspace entfernt. Tool jetzt schließen.",
        "cleanup_confirm": "Temporäre Downloads/extrahierte Tools entfernen? Reports und Logs bleiben erhalten.",
        "cleanup_all_confirm": "Gesamten Workspace inklusive Reports und Logs entfernen?",
...
def relaunch_admin_and_exit():
    if not is_windows() or is_admin():
        return False
    try:
        script = Path(sys.argv[0]).resolve()
        params = " ".join([f'"{script}"'] + [f'"{x}"' if " " in x else x for x in sys.argv[1:]])
...
if is_windows() and not is_admin() and "--no-admin" not in sys.argv:
    relaunch_admin_and_exit()

def safe_workspace():
    root = Path(tempfile.gettempdir()) / "OpenCoreForge_Runtime"
...
def free_drive_letter(preferred="O"):
    if not is_windows():
        return preferred
    used = set()
    try:
...
def run_diskpart_script(script_text):
...
def validate_stick(root):
    ...
    status = "passed"
    for lvl, _ in rows:
        if lvl == "FAIL": status = "failed"; break
        if lvl == "WARN" and status != "failed": status = "warnings"
    return status, rows
...
class Splash(tk.Toplevel):
    def __init__(self, master):
        ...
        self.pb = ttk.Progressbar(self, mode="indeterminate", length=390)
        self.pb.pack(pady=8)
        self.pb.start(8)
        self.status = tk.Label(self, text=str(BASE), bg="#f3f4f6", fg="#4b5563", wraplength=440, font=("Segoe UI", 9))
...
class App(tk.Tk):
    ...
    def create_environment_report(self):
        report = {"app": APP_NAME, "version": APP_VERSION, "python": sys.version, "executable": sys.executable, "workspace": str(BASE), "admin": is_admin(), "platform": sys.platform, "time": time.strftime("%Y-%m-%d %H:%M:%S")}
        (REPORTS / "environment_report.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")

    def robust_download(self, url, target, tries=3):
        headers = {"User-Agent": "Mozilla/5.0 OpenCoreForge/3.2"}
        last = None
    ...
    def download_tools(self):
        for name, url in SOURCES.items():
            ...
            if target.exists() and target.stat().st_size > 0:
                continue
            ok, err = self.robust_download(url, target)
            if not ok:
        ...
        for z in DOWNLOADS.glob("*.zip"):
            ...
            try:
                ...
                self.download_errors.append(f"extract {z.name}: {e}")
    ...
    def set_language_and_show(self, lang):
        ...
        self.deiconify()
        if self.download_errors:
            self.write_status(self.text("download_warn"))
    ...
    def build_main(self):
        ...
        ttk.Label(main, text=self.text("status_ready"), style="Small.TLabel").grid(row=1, column=0, sticky="w", pady=(0, 10))
        form = ttk.Frame(main)
        ...
        self.adv_btn = ttk.Checkbutton(main, text="▼ " + self.text("advanced"), variable=self.adv_open, command=self.toggle_advanced)
        ...
        ttk.Checkbutton(self.adv, text=self.text("save_folder"), variable=self.save_folder_var, command=self.update_flash_text).grid(row=0, column=0, columnspan=3, sticky="w", pady=3)
        self.add_option(self.adv, 1, self.text("secure"), self.secure_var, ["Disabled", "Default"])
        self.add_option(self.adv, 2, self.text("scan"), self.scan_var, ["0", "Default"])
        self.add_option(self.adv, 3, self.text("hide"), self.hide_var, ["False", "True"])
        ttk.Checkbutton(self.adv, text=self.text("verbose"), variable=self.verbose_var).grid(row=4, column=1, sticky="w", pady=3)
        ttk.Label(self.adv, text=self.text("efi")).grid(row=5, column=0, sticky="w", pady=3)
        ...
        ttk.Button(self.adv, text=self.text("browse"), command=self.pick_efi).grid(row=5, column=2, padx=4)
        ttk.Label(self.adv, text=self.text("recovery")).grid(row=6, column=0, sticky="w", pady=3)
        self.rec_label = ttk.Label(self.adv, text=str(self.recovery) if self.recovery else "-", width=42)
        self.rec_label.grid(row=6, column=1, sticky="ew", pady=3)
        ttk.Button(self.adv, text=self.text("browse"), command=self.pick_recovery).grid(row=6, column=2, padx=4)
        ttk.Button(self.adv, text=self.text("auto_find"), command=self.refresh_find).grid(row=7, column=1, sticky="w", pady=4)
        self.status = tk.Text(main, height=8, width=70, bg="#111827", fg="#e5e7eb", relief="flat", font=("Consolas", 9), wrap="word")
        ...
        ttk.Button(bottom, text=self.text("cleanup"), command=self.cleanup).grid(row=0, column=1, sticky="e", padx=8)
        ttk.Button(bottom, text=self.text("workspace"), command=self.open_workspace).grid(row=0, column=2, sticky="e", padx=8)
    ...
    def update_flash_text(self):
        self.flash_button.config(text=self.text("save") if self.save_folder_var.get() else self.text("flash"))
    ...
    def select_device(self):
        ...
        self.selected_usb = self.usb[i] if 0 <= i < len(self.usb) else None
    ...
    def flash(self):
        if not self.selected_macos:
            messagebox.showwarning(APP_NAME, "No macOS selected."); return
        if not self.efi or not Path(self.efi).exists():
            messagebox.showerror(APP_NAME, "No EFI selected/found."); return
        if self.save_folder_var.get():
            target = filedialog.askdirectory(title=self.text("save_folder"))
            if not target: return
            threading.Thread(target=lambda: self.save_to_folder(Path(target)), daemon=True).start()
            return
        if not self.selected_usb:
            messagebox.showwarning(APP_NAME, "No USB device selected."); return
        if is_windows() and not is_admin():
            messagebox.showinfo(APP_NAME, self.text("admin"))
            relaunch_admin_and_exit()
            return
        if simpledialog.askstring(APP_NAME, self.text("erase")) != "ERASE":
            self.write_status("Cancelled."); return
        threading.Thread(target=self.flash_worker, daemon=True).start()

    def save_to_folder(self, target):
        try:
            self.put(self.write_status, self.text("status_save"))
            efi_out = target / "EFI"
            ...
            self.put(self.write_status, self.text("saved_to") + ": " + str(target))
            self.put(messagebox.showinfo, APP_NAME, self.text("saved_to") + "\n" + str(target))
        except Exception:
            self.handle_error()
    ...
    def write_report(self, target):
        ...
        msg = self.text("report_ok") if status == "passed" else self.text("report_warn") if status == "warnings" else self.text("report_fail")
    ...
    def format_usb(self):
        ...
        if num is None or str(num).strip() == "":
            raise RuntimeError("No valid disk number selected")
        script = (
            f"select disk {num}\n"
            "detail disk\n"
            "clean\n"
            ...
            f"format fs=fat32 quick label=OPENCORE\n"
            f"assign letter={letter}\n"
            "exit\n"
        )
        r = run_diskpart_script(script)
        self.put(self.write_status, (r.stdout or "") + "\n" + (r.stderr or ""))
        target = Path(f"{letter}:/")
        if r.returncode != 0:
            raise RuntimeError(f"diskpart failed with code {r.returncode}. See logs: {LOGS / 'diskpart_last_stdout.log'}")
        for _ in range(20):
            if target.exists():
                return target
            time.sleep(0.25)
    ...
    def cleanup(self):
        if messagebox.askyesno(APP_NAME, self.text("cleanup_confirm")):
            for p in [DOWNLOADS, EXTRACTED, TOOLS, OUTPUT]:
                if p.exists(): shutil.rmtree(p, ignore_errors=True)
                p.mkdir(parents=True, exist_ok=True)
            self.write_status(self.text("cleanup_done"))
        if messagebox.askyesno(APP_NAME, self.text("cleanup_all_confirm")):
            shutil.rmtree(BASE, ignore_errors=True)
            messagebox.showinfo(APP_NAME, self.text("cleanup_all_done"))
